...
if uploaded_file:
    ...
    if all(col in df.columns for col in required_cols):
        ...
        if df_clean.empty:
            ...
            # Optimisation par nearest neighbour
            def optimiser(df_in):
                seq = [df_in.iloc[0]]
                ...
                while not reste.empty:
                    ...
                    idx = reste.apply(lambda row: geodesic(origin,(row["Latitude"],row["Longitude\])).meters, axis=1).idxmin()
                    seq.append(reste.loc[idx]); reste = reste.drop(idx)
                return pd.DataFrame(seq)

            df_opt = optimiser(df_clean)
            st.subheader("Liste organisée")
            st.dataframe(df_opt[["Adresse du client","CPSTCMN","LVIL","Latitude","Longitude"]], use_container_width=True)

            # Appel unique OSRM pour route simplifiée
            waypoints = ";".join(f"{lon},{lat}" for lat,lon in zip(df_opt["Latitude"], df_opt["Longitude"]))
            ...
            def make_map(route_pts, markers, labels):
                ...
                m.add_child(line)
                PolyLineTextPath(line, '▶', repeat=True, offset=12, attributes={'fill':'blue','font-weight':'bold','font-size':'14'})
                for i,(lat,lon) in enumerate(markers):
                    color = 'green' if i==0 else ('red' if i==len(markers)-1 else 'blue')
                    icon = folium.DivIcon(html=f"<div style='background:{color};border-radius:50%;width:28px;height:28px;"
                                               f"display:flex;align-items:center;justify-content:center;color:white;font-weight:bold'>"
                                               f"{i+1}</div>")
                    folium.Marker(location=(lat,lon), icon=icon, tooltip=labels[i], popup=labels[i]).add_to(m)
                return m

            map_obj = make_map(route_pts, list(zip(df_opt["Latitude"], df_opt["Longitude"])),
            ...
            output.seek(0)
            st.download_button("Télécharger en XLSX", data=output,
                               file_name="repérage_organisé.xlsx",
                               mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    else:
        st.error(f"Colonnes manquantes: {', '.join(set(required_cols)-set(df.columns))}")
